...
export default function NewRetreatSaving() {
    ...
    useEffect(() => {
        if (preselectedYouthId && youths.length > 0) {
            const youth = youths.find(y => y.id === preselectedYouthId);
            if (youth) setSelectedYouth(youth);
        }
    }, [preselectedYouthId, youths]);

    const filteredYouths = youths.filter(y => y.name.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        <ScrollView style={styles.container} scrollEnabled={scrollEnabled}>
            <View style={styles.form}>
                ...
                <View style={styles.signatureButtons}>
                    ...
                    <TouchableOpacity style={[styles.saveButton, isPending && styles.disabled]} onPress={handleSign} disabled={isPending}>
                        <Text style={styles.saveButtonText}>{isPending ? 'Guardando...' : 'Confirmar y Guardar'}</Text>
                    </TouchableOpacity>
                </View>
            </View>
            ...
            <Modal visible={modalVisible} animationType="slide" presentationStyle="pageSheet">
                <View style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        ...
                        <TouchableOpacity onPress={() => setModalVisible(false)}><Text style={styles.closeModalText}>Cerrar</Text></TouchableOpacity>
                    </View>
                    <View style={styles.searchContainer}>
                        <Search color="#9CA3AF" size={20} />
                        <TextInput style={styles.searchInput} placeholder="Buscar por nombre..." value={searchQuery} onChangeText={setSearchQuery} />
                    </View>
                    <FlatList
                        data={filteredYouths}
...
const styles = StyleSheet.create({
    ...
    selector: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: '#D1D5DB' },
    selectorText: { fontSize: 16, color: '#1F2937' },
    input: { backgroundColor: '#fff', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: '#D1D5DB', fontSize: 16 },
    signatureContainer: { height: 200, backgroundColor: '#fff', borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: '#D1D5DB', marginTop: 8 },
    signatureButtons: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 16, gap: 12 },
    clearButton: { flex: 1, padding: 16, backgroundColor: '#FEE2E2', borderRadius: 12, alignItems: 'center' },
    clearButtonText: { color: '#EF4444', fontWeight: 'bold', fontSize: 16 },
    saveButton: { flex: 2, padding: 16, backgroundColor: '#8B5CF6', borderRadius: 12, alignItems: 'center' },
    ...
    searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', margin: 20, paddingHorizontal: 16, borderRadius: 12, borderWidth: 1, borderColor: '#D1D5DB' },
    searchInput: { flex: 1, paddingVertical: 12, marginLeft: 8, fontSize: 16 },
    modalItem: { backgroundColor: '#fff', padding: 16, marginHorizontal: 20, marginBottom: 8, borderRadius: 12 },
