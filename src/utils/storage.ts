...
export const migrateLocalDataToCloud = async (): Promise<{
  success: boolean;
  ...
  try {
    ...
    if (!session) {
      ...
      return {
        success: false,
        migratedCount: 0,
        message: 'Inicia sesión para sincronizar tus datos.',
      };
    }
    ...
    console.log(
      `Encontrados ${localPeople.length} miembros y ${localPayments.length} pagos locales.`
    );
    ...
    if (cloudErr) {
      ...
      return {
        ...
        migratedCount: 0,
        message:
          'No se pudo conectar a la nube. Verifica tu conexión a internet e inténtalo de nuevo.',
      };
    }
    ...
    for (const person of localPeople) {
      ...
      if (existingPerson) {
        ...
        console.log(
          `"${person.name}" ya existe en la nube (match: ${existingPerson.name}), omitiendo duplicado.`
        );
    ...
    const paymentsToInsert = localPayments
      ...
      .filter(p => {
        ...
        if (isDuplicate) {
          console.log(
            `Pago duplicado detectado (persona: ${p.person_id}, mes: ${p.month}, monto: ${p.amount}), omitiendo.`
          );
        }
        return !isDuplicate;
      });
    ...
    if (!hadErrors && allPeopleMapped) {
      ...
      console.log(
        '--- Migración completada con éxito al 100%. Datos locales eliminados limpiamente. ---'
      );

      const msg =
        newPeopleCount > 0 || newPaymentsCount > 0
      ...
      console.log(
        '--- Migración parcial o con errores. No se borró la caché local para evitar pérdida de datos. ---'
      );
      return {
        success: false,
        migratedCount: Object.keys(idMap).length,
        message:
          'Algunos datos no se pudieron migrar. Tus datos locales siguen intactos. Intenta de nuevo con buena conexión a internet.',
      };
    }
  } catch (e) {
    ...
    return {
      success: false,
      migratedCount: 0,
      message: 'Error de conexión. Tus datos locales siguen intactos en tu celular.',
    };
  }
};
...
export const deletePayment = PaymentRepository.delete;
