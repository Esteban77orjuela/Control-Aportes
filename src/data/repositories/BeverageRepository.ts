...
export const BeverageSalesRepository = {
    ...
    getDashboardStats: async () => {
        try {
            const [beverages, sales] = await Promise.all([BeverageRepository.getAll(), BeverageSalesRepository.getAll()]);

            const totalProducts = beverages.length;
            const totalStock = beverages.reduce((sum, b) => sum + b.stock, 0);
            const inventoryValue = beverages.reduce((sum, b) => roundMoney(sum + (b.stock * b.costPrice)), 0);
