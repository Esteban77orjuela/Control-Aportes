...
import { Package, PlusCircle, DollarSign } from 'lucide-react';
...
export default function RefillStock() {
  ...
  const params = useLocalSearchParams<{ beverageId?: string }>();

  const [loading, setLoading] = useState(true);
  const [beverages, setBeverages] = useState<Beverage[]>([]);
  const [selectedId, setSelectedId] = useState<string | null>(params.beverageId || null);
  const [quantityToAdd, setQuantityToAdd] = useState('');
  const [newCostPrice, setNewCostPrice] = useState('');
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    getBeverages().then(data => {
      setBeverages(data);
      if (params.beverageId) {
        const selected = data.find(b => b.id === params.beverageId);
        if (selected) {
    ...
    >
      ...
      >
        ...
        <Text style={styles.formSubtitle}>
          Suma unidades a un producto que ya existe en tu catálogo.
        </Text>
        ...
        <View style={styles.selectorContainer}>
          {beverages.map(bev => (
            <TouchableOpacity
              key={bev.id}
              style={[styles.option, selectedId === bev.id && styles.optionSelected]}
            ...
            <View style={styles.summaryCard}>
              <Text style={styles.summaryText}>
                ...
                <Text style={styles.bold}>
                  {(selectedBeverage?.stock || 0) + (Number(quantityToAdd) || 0)}
                </Text>{' '}
                unidades.
              </Text>
            </View>
